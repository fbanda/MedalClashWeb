...
export const SideCard = (props: SideCardProps) => {
  const {cardId, cardType, medalLevel, image, name, cardCode, amount, hideHexagon, isMainDeck, medalReq1, medalReq2, medalReq3, card} = props;
  const store = useStore();

  const deckCard = store.deck.cards.find(c => c.id === cardId);
      ...
      <div className={"flex gap-4"}>
        ...
        <div className={"flex flex-col"}>
          <div className={"flex gap-1 font-bold leading-4"}>
            {deckCard?.isError ? <div><ExclamationIcon/></div> : ""}<div>{name}{cardType === "Leader" ? " (Leader)" : ""}{medalLevel ? " Lv" + medalLevel : ""}</div>
          </div>
          <div>
        ...
        <div className={"flex flex-row gap-1 ml-auto"}>
          <Hexagon size={"sm"} isBtn text={<EyeIcon/>} onClick={() => {
